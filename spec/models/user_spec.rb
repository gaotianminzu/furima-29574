...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    context '新規登録がうまくいくとき' do
      ...
      it 'first_name, last_nameが全角（漢字）であれば登録できる' do
        @user.first_name = '明子'
        @user.last_name = '高田'
        expect(@user).to be_valid
      end
      it 'first_name, last_nameが全角（ひらがな）であれば登録できる' do
        @user.first_name = 'あきこ'
        @user.last_name = 'たかだ'
        expect(@user).to be_valid
      end
      it 'first_name, last_nameが全角（カタカナ）であれば登録できる' do
        @user.first_name = 'アキコ'
        @user.last_name = 'タカダ'
        expect(@user).to be_valid
      end
      it 'first_name_kana, last_name_kanaが 全角（カタカナ）であれば、登録できる。' do
        @user.first_name = 'タカダ'
        @user.last_name = 'アキコ'
        expect(@user).to be_valid
      end
    ...
    context '新規登録がうまくいかないとき' do
      ...
      it 'first_name, last_nameが全角ではない場合、登録できない（すべてローマ字）' do
      ...
      it 'first_name, last_nameが全角ではない場合、登録できない（すべて半角カタカナ）' do
        @user.first_name = 'ｱｷｺ'
        @user.last_name = 'ﾀｶﾀﾞ'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name は全角で入力してください。', 'Last name は全角で入力してください。')
      end
      it 'first_name, last_nameが全角ではない場合、登録できない（全て数字）' do
        @user.first_name = '123'
        @user.last_name = '456'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name は全角で入力してください。', 'Last name は全角で入力してください。')
      end
      it 'first_name_kana, last_name_kanaが 全角（カタカナ）ではない場合、登録できない（全てひらがな）' do
      ...
      it 'first_name_kana, last_name_kanaが 全角（カタカナ）ではない場合、登録できない（全て漢字）' do
        @user.first_name_kana = '明子'
        @user.last_name_kana = '高田'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name kana は全角カタカナで入力して下さい。', 'Last name kana は全角カタカナで入力して下さい。')
      end
      it 'first_name_kana, last_name_kanaが 全角（カタカナ）ではない場合、登録できない（全て数字）' do
